...
import {
    Plus,
    ChevronLeft,
    ChevronRight, // FIXED: Capitalized
    Trash2,
    ArrowLeft,
    Sparkles,
    Brain,
...
// FIXED: Destructured the prop correctly
const FlashcardManager = ({ documentId }) => {
    ...
    const handleReview = async (index) => {
        ...
        try {
            ...
            // Optional: Remove toast so it doesn't spam the user on every flip
            ...
            return (
                <div className="flex flex-col items-center justify-center min-h-[400px] text-center p-8 border border-white/5 border-dashed rounded-[40px] bg-white/1">
                    <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-white/5 flex items-center justify-center mb-6 shadow-2xl">
                        <Brain className="text-emerald-500" size={32} strokeWidth={1.5} />
                    </div>
                    ...
                    <button
                        ...
                        disabled={generating}
                        className="py-3.5 px-6 bg-emerald-500 hover:bg-emerald-400 text-black rounded-xl font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 active:scale-[0.98]"
                    >
                        {generating ? (
                            <>
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-2">
                    ...
                    <button
                        onClick={handleGenerateFlashcards}
                        disabled={generating}
                        className="py-3 px-5 bg-white/5 hover:bg-white/10 text-white border border-white/5 rounded-xl font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2 active:scale-[0.98]"
                    >
                        {generating ? (
                            <>
                    ...
                    {flashcardSets.map((set) => (
                        ...
                        >
                            ...
                            <div className='flex-1 flex flex-col relative z-10 mt-2'>
                                <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center mb-5 shadow-inner">
                                    <Brain size={24} strokeWidth={2} />
                                </div>

                                <div className="mb-6">
                                    <h4 className="text-lg font-bold text-white mb-1 tracking-tight group-hover:text-emerald-400 transition-colors">
                                        Flashcard Set
                                    </h4>
                                ...
                                <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
                                    ...
                                    <ChevronRight size={18} className="text-zinc-600 group-hover:text-emerald-400 group-hover:translate-x-1 transition-all" />
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="space-y-8">
                    ...
                    <div className="flex items-center gap-4 justify-end pt-4 border-t border-white/5">
                        ...
                        <button
                            ...
                            disabled={deleting}
                            className="px-6 py-3 rounded-xl font-bold text-sm bg-rose-500 hover:bg-rose-600 text-white shadow-lg shadow-rose-500/20 transition-all disabled:opacity-50 flex items-center gap-2"
                        >
                            {deleting ? (
                                <>
                                    <div className="w-4 h-4 border-2 border-white/20 border-t-white animate-spin rounded-full" />
